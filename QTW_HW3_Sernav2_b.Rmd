...
{
   # create a matrix for spam, ham, and log odds
  wordTable = matrix(0.5, nrow = 4, ncol = length(pooledbow), 
                     dimnames = list(c("spam", "ham", 
                                        "presentLogOdds", 
                                        "absentLogOdds"),  pooledbow))
...
cvMsgWords1 <- msgWordsList
cvMsgWords1 <- cvMsgWords1[sample(length(cvMsgWords1))]
wordFoldTestIndexes <- createFolds(cvMsgWords1, k=5, list=TRUE)
...
#not sure if this is the correct approach for isSpam vector
IsSpamFold1 = rep(c(TRUE, FALSE), c(length(wordFoldTestIndexes1), length(wordFoldTestIndexes1)))
IsSpamFold2 = rep(c(TRUE, FALSE), c(length(wordFoldTestIndexes2), length(wordFoldTestIndexes2)))
IsSpamFold3 = rep(c(TRUE, FALSE), c(length(wordFoldTestIndexes3), length(wordFoldTestIndexes3)))
IsSpamFold4 = rep(c(TRUE, FALSE), c(length(wordFoldTestIndexes4), length(wordFoldTestIndexes4)))
IsSpamFold5 = rep(c(TRUE, FALSE), c(length(wordFoldTestIndexes5), length(wordFoldTestIndexes5)))

wordTestFold1 <- cvMsgWords1[wordFoldTestIndexes1]
wordTrainFold1 <- cvMsgWords1[-wordFoldTestIndexes1]
wordTestFold2 <- cvMsgWords1[wordFoldTestIndexes2]
wordTrainFold2 <- cvMsgWords1[-wordFoldTestIndexes2]
wordTestFold3 <- cvMsgWords1[wordFoldTestIndexes3]
wordTrainFold3 <- cvMsgWords1[-wordFoldTestIndexes3]
wordTestFold4 <- cvMsgWords1[wordFoldTestIndexes4]
wordTrainFold4 <- cvMsgWords1[-wordFoldTestIndexes4]
wordTestFold5 <- cvMsgWords1[wordFoldTestIndexes5]
wordTrainFold5 <- cvMsgWords1[-wordFoldTestIndexes5]

pooledbow = unique(unlist(wordTrainFold5))
pooledbow = append(pooledbow, unique(unlist(wordTrainFold4)))
pooledbow = append(pooledbow, unique(unlist(wordTrainFold3)))
pooledbow = append(pooledbow, unique(unlist(wordTrainFold2)))
pooledbow = append(pooledbow, unique(unlist(wordTrainFold1)))

spamWordCounts1 = rep(0, length(pooledbow))
names(spamWordCounts1) = pooledbow

trainTable1 = computeFreqs1(wordTrainFold1, IsSpamFold1)
testLLRFold1 = sapply(wordTestFold1, computeMsgLLR, trainTable1)

trainTable2 = computeFreqs1(wordTrainFold2, IsSpamFold2)
testLLRFold2 = sapply(wordTestFold2, computeMsgLLR, trainTable2)

trainTable3 = computeFreqs1(wordTrainFold3, IsSpamFold3)
testLLRFold3 = sapply(wordTestFold3, computeMsgLLR, trainTable3)

trainTable4 = computeFreqs1(wordTrainFold4, IsSpamFold4)
testLLRFold4 = sapply(wordTestFold4, computeMsgLLR, trainTable4)

trainTable5 = computeFreqs1(wordTrainFold5, IsSpamFold5)
testLLRFold5 = sapply(wordTestFold5, computeMsgLLR, trainTable5)
...
#need to figure out correct isSpam vector to send here.
cvTypeOneErrorRate = typeIErrorRates(pooledLLR, IsSpam)
cvTypeOneErrorRate
